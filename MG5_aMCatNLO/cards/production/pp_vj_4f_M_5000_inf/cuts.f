...
c     compute total wj's momentum
      do i=nincoming+1,nexternal
         do j=0,3
            ptemp(j) = p(j,i) + ptemp(j)
         enddo
      enddo

      mbs = dsqrt(ptemp(0)**2-ptemp(1)**2-ptemp(2)**2-ptemp(3)**2)

c      write (*,*) mbs,' -> mbbbb'
      if (mbs.lt.5000d0) then
         passcuts=.false.
      endif
...
c      if(nheavyjets.gt.0) then
c         do i=1,nheavyjets
c            inclht=inclht+ptheavyjet(i)
c         enddo
c      endif !if there are heavyjets

c  ----------------------
c     Modified inclht cut to cut on scalar sum of all parton pT
c  ----------------------
      inclht = 0
      do i=nincoming+1,nexternal
         inclht=inclht+pt(p(0,i))
      enddo
