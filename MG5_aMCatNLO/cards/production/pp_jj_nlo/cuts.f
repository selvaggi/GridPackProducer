...
c$$$C pass cuts)
c            if ( p(1,i)**2+p(2,i)**2 .lt. 2500d0**2 ) then
...
c     compute hardest pT
      
      pt1 = - 999.
      do i=1,nexternal
         if (istatus(i).eq.1) then 
            if ( dsqrt(p(1,i)**2+p(2,i)**2) .gt. pt1 ) then
	       pt1 = dsqrt(p(1,i)**2+p(2,i)**2)
	       i1 = i
	    endif
	 endif
      enddo

c     compute second leading pT
      
      pt2 = - 999.
      do i=1,nexternal
         if (istatus(i).eq.1) then 
            if ( dsqrt(p(1,i)**2+p(2,i)**2) .gt. pt2 ) then
                if ( dsqrt(p(1,i)**2+p(2,i)**2) .lt. pt1 ) then
	           pt2 = dsqrt(p(1,i)**2+p(2,i)**2)
	           i2 = i
	        endif
	    endif
	 endif
...
c     compute mjj for two leading jets
      
      mjj = dsqrt( (p(0,i1) + p(0,i2))**2 - (p(1,i1) + p(1,i2))**2 - (p(2,i1) + p(2,i2))**2 - (p(3,i1) + p(3,i2))**2 )
      if (mjj .lt. 5000d0) then
          passcuts_user=.false.
          return
      endif
